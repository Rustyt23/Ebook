...
struct PopularBooks: View {
    @State  var width : CGFloat = 300
    @ObservedObject var manager = popular_request()
    @ObservedObject var sender = favorite_post()
    @ObservedObject var getbook = BookDetail_request()
                            ...
                            ForEach(self.manager.jsonData.data!, id: \._id) { s in
                                VStack(alignment:.leading){
                                    ZStack(alignment:.bottomLeading){
                                        ...
                                        {
                                            NavigationLink(
                                                destination: BookDetail(bookid: s._id)
                                        ...
                                        HStack{
                                            ...
                                            HStack{
                                                Image(systemName: "star.fill").foregroundColor(Color("gold"))
                                                    .frame(width:(UIScreen.main.bounds.width - self.width)/6, height: (UIScreen.main.bounds.width - self.width)/6 )
                                                Text(String(s.rating)).foregroundColor(.white).fixedSize() .frame(width:(UIScreen.main.bounds.width - self.width)/6, height: (UIScreen.main.bounds.width - self.width)/6 ).padding(.trailing,8).font(/*@START_MENU_TOKEN@*/.title2/*@END_MENU_TOKEN@*/)
                                            }
...
class popular_request: ObservableObject{
    @Published var jsonData =  Banner_model()
    @Published var auth = false
    func getAuth() {
        guard let url = URL(string: "http://143.110.185.192:5000/api/book/popular_books") else { print("URL not working")
            return
        }
